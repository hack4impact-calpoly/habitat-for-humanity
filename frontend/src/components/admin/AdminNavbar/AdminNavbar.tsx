...
const AdminNavbar = (): JSX.Element => {
    ...
    const underline = (header: string): boolean => {
        if (header === navBarHeaders[2] && (page_path.includes(donationApprovalsPath))) { // For different donation pages
            return true;
        } 
    ...
    const navlinkHandler = (header : string) : string => {
        ...
        else if (header === navBarHeaders[2]){
            return tempPath;
        }
        else if (header === navBarHeaders[3]){
            return tempPath;
        }
        else if (header === navBarHeaders[4]){
            return tempPath;
        }
        else if (header === navBarHeaders[5]){
            return signoutPath;
        }
        else{
                ...
                {navBarHeaders?.map((header: string, index: number): JSX.Element => {
                    return underline(header) ? (
                        <div className="adminNavbarHeader" key={index}>
                            <Link id="adminNavbarUnderline" className="adminNavbarLink" to={navlinkHandler(header)}>{header}</Link>
                        </div> ) : ( 
                            <Link key={index} className="adminNavbarLink" to={navlinkHandler(header)}>{header}</Link>
                        )
                })}
            </div>
        </div>
